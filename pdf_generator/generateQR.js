...
function generateQRCodeAndPDF(id,phone,randomNumber,email,departure,arrival,departure_time,arrival_time,arrivalDate,Date,bus_number,routenumber,price,seatNumbers) {
  ...
  return new Promise((resolve, reject) => {
    ...
    QRCode.toFile('qrcode.png', qrCodeData, function (err) {
      if (err) {
        ...
        doc.rect(40, 40, doc.page.width - 80, 390).lineWidth(1).strokeColor('#FF7F3E').fillAndStroke('#FFFFFF', '#000000');
        doc.rect(50, 50, doc.page.width - 100, 45).fillColor('#000991').fill();
        doc.image('favIcon.png', 60, 55, { width: 35 });
        doc.font('Helvetica-Bold').fontSize(20).fillColor('#FFFFFF').text('ESeats.lk  Travel Pass', 100, 65, { width: doc.page.width - 200, align: 'center' });
        doc.moveDown();
        ...
        doc.moveTo(50, 260).lineTo(doc.page.width - 50, 260).stroke();
        doc.fontSize(12).fillColor('#000991').text('Departure & Arrival Information', 60, 280);
        doc.fontSize(14).fillColor('grey').text('From:', 60, 310);
        doc.fontSize(14).fillColor('black').text(departure, 170, 310);
        doc.fontSize(14).fillColor('grey').text('To:',(doc.page.width - 120)/2+60, 310);
        doc.fontSize(14).fillColor('black').text(arrival, (doc.page.width - 120)/2 + 150, 310);
        doc.fontSize(14).fillColor('grey').text('Departure Date:', 60, 340);
        doc.fontSize(14).fillColor('black').text(Date, 170, 340);
        doc.fontSize(14).fillColor('grey').text('Arrival Date:',(doc.page.width - 120)/2 + 60, 340);
        doc.fontSize(14).fillColor('black').text(arrivalDate, (doc.page.width - 120)/2 + 150, 340);
        doc.fontSize(14).fillColor('grey').text('Departure Time:', 60, 370);
        doc.fontSize(14).fillColor('black').text(departure_time, 170, 370);
        doc.fontSize(14).fillColor('grey').text('Arrival Time:',(doc.page.width - 120)/2 + 60, 370);
        doc.fontSize(14).fillColor('black').text(arrival_time, (doc.page.width - 120)/2 + 150, 370);
        doc.end();
